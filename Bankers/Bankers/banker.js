        ...
        var resource;
            ...
            function BuildFormFields2($amount) //For Allocation
			{
				...
				for ($i = 0; $i < $amount; $i++) {
                    for($j = 0; $j < res; $j++)
                    {
						...
						$field.innerHTML = '<b><u>Process '+$i+'</u></b><br>';
						$field.setAttribute("style","color:black");	
						$item.appendChild($field);
						}
                        $field = document.createElement('label');
                        $field.innerHTML = "Res: " +$j+'<br>'+'Need';
					    $item.appendChild($field);
			...
			function banker()
			{
						...
						while(1)
						{
                            ...
                            ul1.setAttribute("style","color:black");
							for (i = 0; i < pro; i++)
							{
								var flag = 0
								if(ex[i] == 0)
